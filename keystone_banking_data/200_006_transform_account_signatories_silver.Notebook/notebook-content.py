...
SILVER_LAYER     = "silver"
SOURCE_TABLE     = "lh_bronze_banking_data_modern_data.dbo.bronze_account_signatories"
TARGET_TABLE     = "account_signatories"
...
PIPELINE_NAME  = "200_006_transform_account_signatories_silver"
...
signatories_schema = StructType([
    ...
    # Derived columns
    StructField("is_primary_holder", BooleanType(), True),
    StructField("requires_multiple_signatures", BooleanType(), True),
    StructField("signatory_count_on_account", IntegerType(), True),
...
df = (
    df
    .withColumn("signatory_role",
        F.lower(F.trim(F.col("signatory_role")))
    )
    .withColumn("signing_rule",
        F.lower(F.trim(F.col("signing_rule")))
    )
)
...
df = df.withColumn(
    "effective_date",
    F.coalesce(
        F.to_date("effective_date", "yyyy-MM-dd"),
        F.to_date("effective_date", "dd/MM/yyyy"),
        F.to_date("effective_date", "MM/dd/yyyy")
    )
)
...
# Multi-Signature Flag
df = df.withColumn(
    "requires_multiple_signatures",
    F.col("signing_rule").isin(
        "both_to_sign",
        "dual",
        "tiered"
    )
...
# Signatory Count Per Account
account_signatory_counts = (
    df.groupBy("account_id")
      .agg(
          F.count("*")
           .alias("signatory_count_on_account")
      )
)
...
df = (
    df.join(
        customers.select("customer_id").withColumn(
            "customer_exists",
            F.lit(True)
        ),
        "customer_id",
...
# ## Null Classification & Validation

# CELL ********************

REQUIRED = ["account_id", "customer_id", "signatory_role", "signing_rule", "is_active"]
...
print()
print("DQ FLAGS")
orphan_cust = final_df.filter(F.col("is_orphan_customer") == True).count()
orphan_acct = final_df.filter(F.col("is_orphan_account") == True).count()
print(f"  Orphan customer_id refs : {orphan_cust:,}")
print(f"  Orphan account_id refs  : {orphan_acct:,}")
...
BUSINESS_KEY = ["account_id", "customer_id", "signatory_role"]
...
print("=" * 65)
print("  SILVER TRANSFORM SUMMARY — account_signatories")
print("=" * 65)
print(f"""
...
print("── Signatory role distribution ──")
dim.groupBy("signatory_role").count().orderBy("count", ascending=False).show()

print("── Signing rule distribution ──")
dim.groupBy("signing_rule").count().orderBy("count", ascending=False).show()

print("── Multi-signature requirement ──")
dim.groupBy("requires_multiple_signatures").count().show()

print("── Orphan reference check ──")
print(f"  Orphan customer_id : {dim.filter(F.col('is_orphan_customer') == True).count():,}")
