...
SILVER_LAYER     = "silver"
SOURCE_TABLE     = "lh_bronze_banking_data_modern_data.dbo.bronze_account_product_enrollments"
TARGET_TABLE     = "account_product_enrollments"
...
PIPELINE_NAME  = "200_014_transform_account_product_enrollments_silver"
...
enrollments_schema = StructType([
    ...
    # Derived columns
    StructField("is_active_enrollment", BooleanType(), True),
    StructField("is_credit_product", BooleanType(), True),
    StructField("product_count_on_account", IntegerType(), True),
    StructField("enrollment_sequence_on_account", IntegerType(), True),
...
df = (
    ...
    .withColumn("product_code",
        F.lower(F.trim(F.col("product_code")))
    )
...
df = df.withColumn(
    "enrollment_date",
    F.coalesce(
        F.to_date("enrollment_date", "yyyy-MM-dd"),
        F.to_date("enrollment_date", "dd/MM/yyyy"),
        F.to_date("enrollment_date", "MM/dd/yyyy")
    )
)
...
account_product_counts = (
    df.groupBy("account_id")
      .agg(
          F.count("*")
           .alias("product_count_on_account")
      )
)
...
# Enrollment Sequence Per Account (chronological order of enrollments)
account_window = Window.partitionBy("account_id").orderBy("enrollment_date")

df = df.withColumn(
    "enrollment_sequence_on_account",
...
BUSINESS_KEY = ["account_id", "enrollment_date", "product_code"]
...
print("=" * 65)
print("  SILVER TRANSFORM SUMMARY — account_product_enrollments")
print("=" * 65)
print(f"""
...
dim.groupBy("product_code").count().orderBy("count", ascending=False).show()

print("── Enrollment status distribution ──")
dim.groupBy("enrollment_status").count().orderBy("count", ascending=False).show()

print("── Credit product enrollments ──")
dim.groupBy("is_credit_product").count().show()

print("── Products per account distribution ──")
dim.groupBy("product_count_on_account").count().orderBy("product_count_on_account").show()
