...
SILVER_LAYER     = "silver"
SOURCE_TABLE     = "lh_bronze_banking_data_modern_data.dbo.bronze_account_limits_history"
TARGET_TABLE     = "account_limits_history"
...
PIPELINE_NAME  = "200_013_transform_account_limits_history_silver"
...
limits_history_schema = StructType([
    ...
    # Derived columns
    StructField("is_risk_driven_change", BooleanType(), True),
    StructField("total_limit_exposure", DoubleType(), True),
    StructField("change_sequence_on_account", IntegerType(), True),
    StructField("overdraft_limit_delta", DoubleType(), True),
    StructField("credit_card_limit_delta", DoubleType(), True),
...
df = (
    ...
    .withColumn("change_reason",
        F.lower(F.trim(F.col("change_reason")))
    )
)
...
# Risk-Driven Change Flag
df = df.withColumn(
    "is_risk_driven_change",
    F.col("change_reason") == "risk_review"
)

# Total Limit Exposure
df = df.withColumn(
    "total_limit_exposure",
    F.coalesce(F.col("overdraft_limit"), F.lit(0.0)) +
    F.coalesce(F.col("credit_card_limit"), F.lit(0.0))
)

# Change Sequence Per Account (chronological order of limit changes)
account_window = Window.partitionBy("account_id").orderBy("event_date")

df = df.withColumn(
    "change_sequence_on_account",
    F.row_number().over(account_window)
)

# Limit Deltas vs Previous Event (per account, chronological)
df = (
    df.withColumn(
        "prev_overdraft_limit",
        F.lag("overdraft_limit").over(account_window)
    )
    .withColumn(
        "prev_credit_card_limit",
        F.lag("credit_card_limit").over(account_window)
    )
    .withColumn(
        "overdraft_limit_delta",
        F.col("overdraft_limit") - F.col("prev_overdraft_limit")
    )
    .withColumn(
        "credit_card_limit_delta",
        F.col("credit_card_limit") - F.col("prev_credit_card_limit")
    )
    .drop("prev_overdraft_limit", "prev_credit_card_limit")
)
...
BUSINESS_KEY = ["account_id", "event_date"]
...
print("=" * 65)
print("  SILVER TRANSFORM SUMMARY — account_limits_history")
print("=" * 65)
print(f"""
...
print("── Change reason distribution ──")
dim.groupBy("change_reason").count().orderBy("count", ascending=False).show()

print("── Risk-driven changes ──")
dim.groupBy("is_risk_driven_change").count().show()

print("── Limit summary stats ──")
dim.select("overdraft_limit", "credit_card_limit", "total_limit_exposure").describe().show()
